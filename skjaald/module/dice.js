...
export async function d20Roll({parts=[], data={}, event={}, rollMode=null, template=null, title=null, speaker=null,
  ...
  const _roll = (parts, adv, form) => {
    ...
    if (adv === 1) {
      ...
      messageData.flavor += ` (${game.i18n.localize("SKJAALD.Advantage")})`;
      if ( "flags.skjaald.roll" in messageData ) messageData["flags.skjaald.roll"].advantage = true;
      mods += "kh";
    }
    ...
    // Handle disadvantage
      ...
      nd = 2;
      messageData.flavor += ` (${game.i18n.localize("SKJAALD.Disadvantage")})`;
      if ( "flags.skjaald.roll" in messageData ) messageData["flags.skjaald.roll"].disadvantage = true;
      mods += "kl";
    }
    ...
    if (ability && ability.value) {
      ...
      if (abl) {
        data.mod = abl.mod;
        messageData.flavor += ` (${CONFIG.SKJAALD.abilities[data.ability]})`;
      }
    }
    ...
    if (reliableTalent && roll.dice[0].total < 10) {
      messageData.flavor += ` (${game.i18n.localize("SKJAALD.FlagsReliableTalent")})`;
    }
    return roll;
  };
...
async function _d20RollDialog({template, title, parts, data, rollMode, dialogOptions, roll}={}) {
  ...
  // Render modal dialog
  template = template || "systems/skjaald/templates/chat/roll-dialog.html";
  let dialogData = {
    formula: parts.join(" + "),
    data: data,
    rollMode: rollMode,
...
export async function damageRoll({parts, actor, data, event={}, rollMode=null, template, title, speaker, flavor,
  ...
  const _roll = function(parts, crit, form) {
    ...
    if ( crit === true ) {
      ...
      messageData.flavor += ` (${game.i18n.localize("SKJAALD.Critical")})`;
      if ( "flags.skjaald.roll" in messageData ) messageData["flags.skjaald.roll"].critical = true;
    }
