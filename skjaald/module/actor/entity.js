...
export default class Actor5e extends Actor {
  ...
  _computeEncumbrance(actorData) {
    ...
    if ( game.settings.get("skjaald", "currencyWeight") && actorData.data.currency ) {
      ...
      weight += numCoins / CONFIG.SKJAALD.encumbrance.currencyPerWeight;
    }
    ...
    const max = actorData.data.abilities.str.value * CONFIG.SKJAALD.encumbrance.strMultiplier * mod;
  ...
  rollSkill(skillId, options={}) {
    ...
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("SKJAALD.SkillPromptTitle", {skill: CONFIG.SKJAALD.skills[skillId]}),
      halflingLucky: this.getFlag("skjaald", "halflingLucky"),
      reliableTalent: reliableTalent,
      messageData: {"flags.skjaald.roll": {type: "skill", skillId }}
    });
  ...
  rollAbility(abilityId, options={}) {
    ...
    new Dialog({
      title: game.i18n.format("SKJAALD.AbilityPromptTitle", {ability: label}),
  ...
  rollAbilityTest(abilityId, options={}) {
    ...
    const feats = this.data.flags.skjaald || {};
    if ( feats.remarkableAthlete && SKJAALD.characterFlags.remarkableAthlete.abilities.includes(abilityId) ) {
    ...
    const rollData = mergeObject(options, {
      ...
      messageData: {"flags.skjaald.roll": {type: "ability", abilityId }}
    });
    rollData.speaker = options.speaker || ChatMessage.getSpeaker({actor: this});
    return d20Roll(rollData);
  }
  ...
  rollAbilitySave(abilityId, options={}) {
    ...
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("SKJAALD.SavePromptTitle", {ability: label}),
      halflingLucky: this.getFlag("skjaald", "halflingLucky"),
      messageData: {"flags.skjaald.roll": {type: "save", abilityId }}
    });
    rollData.speaker = options.speaker || ChatMessage.getSpeaker({actor: this});
    return d20Roll(rollData);
  }
  ...
  async rollDeathSave(options={}) {
    ...
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.localize("SKJAALD.DeathSavingThrow"),
      speaker: speaker,
      halflingLucky: this.getFlag("skjaald", "halflingLucky"),
      targetValue: 10,
      messageData: {"flags.skjaald.roll": {type: "death"}}
    });
    ...
    if ( success ) {
      ...
      if ( d20 === 20 ) {
        ...
        await ChatMessage.create({content: game.i18n.format("SKJAALD.DeathSaveCriticalSuccess", {name: this.name}), speaker});
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        await ChatMessage.create({content: game.i18n.format("SKJAALD.DeathSaveSuccess", {name: this.name}), speaker});
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        await ChatMessage.create({content: game.i18n.format("SKJAALD.DeathSaveFailure", {name: this.name}), speaker});
      }
    }
  ...
  async rollHitDie(denomination, {dialog=true}={}) {
    ...
    const title = game.i18n.localize("SKJAALD.HitDiceRoll");
    ...
    const roll = await damageRoll({
      ...
      messageData: {"flags.skjaald.roll": {type: "hitDie"}}
    });
    if ( !roll ) return null;
  ...
  async shortRest({dialog=true, chat=true, autoHD=false, autoHDThreshold=3}={}) {
    ...
    if ( chat ) {
      ...
      switch (game.settings.get("skjaald", "restVariant")) {
        case 'normal': restFlavor = game.i18n.localize("SKJAALD.ShortRestNormal"); break;
        case 'gritty': restFlavor = game.i18n.localize(newDay ? "SKJAALD.ShortRestOvernight" : "SKJAALD.ShortRestGritty"); break;
        case 'epic':  restFlavor = game.i18n.localize("SKJAALD.ShortRestEpic"); break;
      }

      // Summarize the health effects
      let srMessage = "SKJAALD.ShortRestResultShort";
      if ((dhd !== 0) && (dhp !== 0)) srMessage = "SKJAALD.ShortRestResult";
  ...
  async longRest({dialog=true, chat=true, newDay=true}={}) {
    ...
    let restFlavor;
    switch (game.settings.get("skjaald", "restVariant")) {
      case 'normal': restFlavor = game.i18n.localize(newDay ? "SKJAALD.LongRestOvernight" : "SKJAALD.LongRestNormal"); break;
      case 'gritty': restFlavor = game.i18n.localize("SKJAALD.LongRestGritty"); break;
      case 'epic':  restFlavor = game.i18n.localize("SKJAALD.LongRestEpic"); break;
    }

    // Determine the chat message to display
    if ( chat ) {
      let lrMessage = "SKJAALD.LongRestResultShort";
      if((dhp !== 0) && (dhd !== 0)) lrMessage = "SKJAALD.LongRestResult";
      else if ((dhp !== 0) && (dhd === 0)) lrMessage = "SKJAALD.LongRestResultHitPoints";
      else if ((dhp === 0) && (dhd !== 0)) lrMessage = "SKJAALD.LongRestResultHitDice";
      ChatMessage.create({
        user: game.user._id,
        speaker: {actor: this, alias: this.name},
        flavor: restFlavor,
  ...
  convertCurrency() {
    ...
    const convert = CONFIG.SKJAALD.currencyConversion;
