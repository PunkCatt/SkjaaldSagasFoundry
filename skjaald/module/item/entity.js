...
export default class Item5e extends Item {
  ...
  prepareData() {
    ...
    // Feat Items
      ...
      if ( act && (act.type === C.abilityActivationTypes.legendary) ) labels.featType = game.i18n.localize("SKJAALD.LegendaryActionLabel");
      else if ( act && (act.type === C.abilityActivationTypes.lair) ) labels.featType = game.i18n.localize("SKJAALD.LairActionLabel");
      else if ( act && act.type ) labels.featType = game.i18n.localize(data.damage.length ? "SKJAALD.Attack" : "SKJAALD.Action");
      else labels.featType = game.i18n.localize("SKJAALD.Passive");
    }
  ...
  _getUsageUpdates({consumeQuantity=false, consumeRecharge=false, consumeResource=false, consumeSpellSlot=false, consumeUsage=false}) {
    ...
    if ( consumeSpellSlot ) {
      ...
      if ( spells === 0 ) {
        const label = game.i18n.localize(consumeSpellSlot === "pact" ? "SKJAALD.SpellProgPact" : `SKJAALD.SpellLevel${id.level}`);
  ...
  async displayCard({rollMode, createMessage=true}={}) {
    ...
    const template = `systems/skjaald/templates/chat/${templateType}-card.html`;
  ...
  getChatData(htmlOptions={}) {
    ...
    if ( data.hasOwnProperty("equipped") && !["loot", "tool"].includes(this.data.type) ) {
      if ( data.attunement === CONFIG.SKJAALD.attunementTypes.REQUIRED ) props.push(game.i18n.localize(CONFIG.SKJAALD.attunements[CONFIG.SKJAALD.attunementTypes.REQUIRED]));
      props.push(
  ...
  async rollAttack(options={}) {
    ...
    const rollConfig = mergeObject({
      ...
      messageData: {"flags.skjaald.roll": {type: "attack", itemId: this.id }}
    }, options);
  ...
  rollDamage({critical=false, event=null, spellLevel=null, versatile=false, options={}}={}) {
    ...
    const itemData = this.data.data;
    const actorData = this.actor.data.data;
    const messageData = {"flags.skjaald.roll": {type: "damage", itemId: this.id }};
  ...
  rollToolCheck(options={}) {
    ...
    const rollConfig = mergeObject({
      parts: parts,
      data: rollData,
      template: "systems/skjaald/templates/chat/tool-roll-dialog.html",
      title: title,
      speaker: ChatMessage.getSpeaker({actor: this.actor}),
      flavor: title,
      dialogOptions: {
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = CONFIG.SKJAALD.spellScrollIds[level];
